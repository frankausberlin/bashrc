...
### Bashrc Rules
* VS Code has its own terminal handling and does not use the .bashrc file. If desired, this behavior can be configured by using the '--use-user-bash' parameter with the pyinit script.
* When the .bashrc file is loaded, it activates the Mamba environment last activated with 'act'. This is unless a .venv folder is located in the current directory, in which case that environment is activated.
...
# The code generated by mamba/conda follows.
