...
# function party
###################################################################
# exportadd, exportfolder, jl, adx, nxx, pyini, repair_path
source ~/.bash_lib
export BASH_DEBUG=0 # 0=off, 1=on

###################################################################
# python-mechanic party I
###################################################################
# The code generated by Mamba/Conda is encapsulated in a function. 
# The function call is managed by the environment mechanics.
_init_conda_mamba() {
# <<< my stuff <<<
#  .-"""-.
# / _   _ \
...
#   `---'
# >>>>>>>>>>>>>>>>
} # end of _init_conda_mamba function


###################################################################
# python-mechanic party II
###################################################################
# Force mamba activation if flag is true OR no .venv exists
if [ "$FORCE_MAMBA_INIT" = "true" ] || [ ! -d "./.venv" ]; then
    # deactivate any existing environments
    deactivate 2>/dev/null || true 
    [[ -n "$CONDA_PREFIX" ]] && conda deactivate > /dev/null 2>&1
    [[ -n "$CONDA_PREFIX" ]] && conda deactivate > /dev/null 2>&1

    # Make mamba/conda available and activate the environment from ~/.startenv (or 'base')
    _init_conda_mamba
...
if [ "$BASH_DEBUG" != "0" ]; then
    echo "BASH_DEBUG is enabled. Current conda/mamba environment: $(basename "$CONDA_PREFIX")"
    echo "FORCE_MAMBA_INIT: $FORCE_MAMBA_INIT"
fi
...
# OpenRouter / Anthropic API settings
export ANTHROPIC_BASE_URL="https://openrouter.ai/api"
export ANTHROPIC_AUTH_TOKEN="$OPENROUTER_API_KEY"
export ANTHROPIC_API_KEY="" # Important: Must be explicitly empty

# Set 1 - cheap
export ANTHROPIC_DEFAULT_OPUS_MODEL="xiaomi/mimo-v2-flash"
...
#export ANTHROPIC_DEFAULT_SONNET_MODEL="moonshotai/kimi-k2-0905:exacto"
#export ANTHROPIC_DEFAULT_HAIKU_MODEL="google/gemini-3-flash-preview"

# Set 3 - a bit more powerful
#export ANTHROPIC_DEFAULT_OPUS_MODEL="perplexity/sonar-reasoning-pro"
#export ANTHROPIC_DEFAULT_SONNET_MODEL="moonshotai/kimi-k2-0905:exacto"
...
rlb() { [[ "$1" == "m" ]] && { export FORCE_MAMBA_INIT="true"; deactivate 2>/dev/null; } || export FORCE_MAMBA_INIT="false"; source ~/.bashrc; }
